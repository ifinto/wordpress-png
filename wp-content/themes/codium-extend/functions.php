...
if (function_exists('register_sidebar')) {
     register_sidebar(array(
      ...
      'id'   => 'single-widgets',
      'description'   => 'Widget Area',
      'before_widget' => '<div id="sgtop-add">',
      'after_widget'  => '</div>'
      //'before_title'  => '<h2>',
      //'after_title'   => '</h2>'
     ));
...
if (function_exists('register_sidebar')) {
     register_sidebar(array(
      'name' => 'Single bottom add',
      'id'   => 'single-bot-widgets',
      'description'   => 'Widget Area',
      'before_widget' => '<div id="sgbot-add">',
      'after_widget'  => '</div>'
      //'before_title'  => '<h2>',
      //'after_title'   => '</h2>'
     ));
...
add_filter('the_content', 'addtargetblank');
 function addtargetblank($content) {
        global $post;
        $pattern ="/<a(.*?)href=('|\")(.*?).(bmp|gif|jpeg|jpg|png)('|\")(.*?)>/i";
        $replacement = '<a$1href=$2$3.$4$5 target="_blank" title="'.$post->post_title.'"$6>';
        $content = preg_replace($pattern, $replacement, $content);
        return $content;
 }
/* Источник: http://n-wp.ru/8653 */
