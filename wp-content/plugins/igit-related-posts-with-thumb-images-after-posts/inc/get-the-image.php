...
function IGIT_get_the_image( $args = array() ) {
	...
	/* If there is no cached image, let's see if one exists. */
		...
		if ( !$image && $the_post_thumbnail )
			$image = IGIT_image_by_the_post_id( $args );
...
function IGIT_image_by_the_post_id( $args = array() ) {
	$text = get_post($args['post_id'])->post_content;
	...
	$first = $array[0];
	$src   = wp_upload_dir()['baseurl'] . '/thumb/' . $first;

	return array( 'url' => $src, 'post_thumbnail_id' => $post_thumbnail_id, 'alt' => $alt );
}
