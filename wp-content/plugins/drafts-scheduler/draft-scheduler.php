...
////			wp_enqueue_style( 'ds-sunny', plugins_url( 'css/jquery-ui.theme.min.css', __FILE__ ) );
//		}
					...
					<table class="form-table">
						...
						<tr valign="top">
							...
							<td>
								...
								<input type="text" name="intMins" id="intMins" size="4" maxlength="2"
								       value="<?php echo isset( $_POST['intMins'] ) ? absint( $_POST['intMins'] ) : ''; ?>"/><strong>mins</strong>
							</td>
						</tr>
						...
						<tr valign="top">
							...
							<td>
								...
								Post up to <input type="text" name="postsperday" id="postsperday" value="<?php echo isset( $_POST['postsperday'] ) ? absint( $_POST['postsperday'] ) : ''; ?>" size="3"
								                  maxlength="3"/> posts per
								<select name="dayweekmonth">
									<option value="day">day</option>
									<option value="week">week</option>
									<option value="month">month</option>
								</select> <strong>OR</strong>
								Post exactly <input type="text" name="exactPosts" id="exactPosts" value="<?php echo isset( $_POST['exactPosts'] ) ? absint( $_POST['exactPosts'] ) : ''; ?>" size="3"
								                    maxlength="3"/> posts per day<br/>
								Randomly after:
		...
		function draftScheduler_randomize_drafts( $startDate, $startTime, $endTime, $dailyMax = "", $exactPosts = "", $dayweekmonth ) {
			...
			while ( !$finished ):
				...
				$this->update_post_details( strtotime( $postTime ), $thisDraft );
				$whole_post = get_post( $thisDraft );
				wp_transition_post_status( 'draft', 'future', $whole_post );
				check_and_publish_future_post( $thisDraft );
		...
		function draftScheduler_random_post_by_interval( $startDate, $intvHours, $intvMins ) {
			...
			foreach ( $draftList as $thisDraft ) {
				...
				$this->update_post_details( $ds_postDate, $thisDraft );
				$whole_post = get_post( $thisDraft );
				wp_transition_post_status( 'draft', 'future', $whole_post );
				check_and_publish_future_post( $thisDraft );
...
function theme_admin_js() {
	...
	wp_enqueue_style( 'ds-sunny', "//code.jquery.com/ui/1.11.4/themes/sunny/jquery-ui.css" );
}
